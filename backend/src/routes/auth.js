...
router.get('/me', authMiddleware, (req, res) => {
  ...
  const referrer = member.referrer_id
    ? db.prepare('SELECT id, name, phone FROM members WHERE id = ?').get(member.referrer_id)
    : null;

  const tierProgress = db.prepare('SELECT * FROM tier_progress WHERE member_id = ?').get(member.id);
